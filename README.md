...
Для входа через Shikimori на Vercel должны быть определены переменные окружения `SHIKI_CLIENT_ID`, `SHIKI_CLIENT_SECRET` и `SHIKI_REDIRECT_URI`.
...
Для локального тестирования используйте редирект `http://localhost:5173/auth/callback` и установите `SHIKI_REDIRECT_URI` на этот адрес. В интерфейсе применяются прямые ссылки на `/api/auth/*`, поэтому авторизация работает и при запуске `npm run dev`. На Vercel запросы к `/auth/*` дополнительно перенаправляются на соответствующие серверные функции.
